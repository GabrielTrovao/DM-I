...
ggplot(AQIbydate, aes(day, 
  ...
  theme_classic()              # months closer or within winter tend to have more AQI spikes
...
#make neural network
# need to use different dataset because of normalized AQI
PredictionDBNN <- PredictionDB
PredictionDBNN$PM2.5 <- normalize(PredictionDB$PM2.5)
PredictionDBNN$TEMP <- normalize(PredictionDB$TEMP)
PredictionDBNN$PRES <- normalize(PredictionDB$PRES)
PredictionDBNN$DEWP <- normalize(PredictionDB$DEWP)
PredictionDBNN$RAIN <- normalize(PredictionDB$RAIN)
PredictionDBNN$WSPM <- normalize(PredictionDB$WSPM)
PredictionDBNN$AQI <- normalize(PredictionDB$AQI)

n <- nrow(PredictionDBNN)
trPrec <- 0.7
sp <- sample(1:n, as.integer(trPrec * n))
train <- PredictionDBNN[sp,]
test <- PredictionDBNN[-sp,]
neuraln <- neuralnet(AQI ~ PM2.5 + TEMP + PRES + RAIN
                       + wd_E + wd_ENE + wd_ESE + wd_N + wd_NE + wd_NNE +
                       + wd_NNW + wd_NW + wd_S + wd_SE + wd_SSE + wd_SSW +
                       + wd_SW + wd_W + wd_WNW + wd_WSW + WSPM, data = train, hidden = 2, act.fct = "logistic",linear.output = FALSE)
plot(neuraln)

neuraln
predict <- compute(neuraln, test)

getNNclass <- function(x) {  # since AQI is measured from 0 to 500, then, from 0 to 1, danger arises 
  AQIthreshold <- 0.2        # at the 0.2 mark(which corresponds to AQI level 100)
  i <- "Unhealthy"
  for (variable in x) {
    if(x < AQIthreshold){
      i <- "Healthy"
...
CorrelationNN <- select(test, "date", "AQI")
CorrelationNN$AQILevel <- sapply(CorrelationNN$AQI, FUN = getNNclass)
CorrelationNN$Predictions <- predict$net.result
CorrelationNN$PredsLevel <- sapply(CorrelationNN$Predictions, FUN = getNNclass)
CorrelationNN
LevelAccuracy <- nrow(subset(CorrelationNN,AQILevel == PredsLevel,PredsLevel))/nrow(CorrelationNN)
LevelAccuracy
#nrow(subset(CorrelationNN,PredsLevel==1,PredsLevel))
